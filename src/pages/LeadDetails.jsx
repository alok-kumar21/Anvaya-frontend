...
const LeadDetails = () => {
  ...
  const {
    ...
    loading: commentLoading,
    error: commentError,
  } = useFetch(`http://localhost:5001/leads/${leadId}/comments`);
  ...
  // Initialize local comments state
  useEffect(() => {
    if (commentData) {
  ...
  async function formDataHandler(event) {
    ...
    try {
      const response = await fetch(
        `http://localhost:5001/leads/${leadId}/comments`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          <div className="row">
            ...
            <div className="lead-content p-5 col-md-10">
              <ul className="list mb-5">
                ...
                <li className="list-group-item pt-3">
                  Sales Agent: {leaddetails?.salesAgent?.name || "Unassigned"}
                </li>
                <li className="list-group-item pt-3">
                  Lead Source: {leaddetails.source}
                </li>
              ...
              {commentError && (
                <div className="text-danger">Failed to load comments</div>
              )}
              ...
              <ul className="list mt-3">
                {comments?.length === 0 ? (
                  ...
                  comments.map((comment) => (
                    <li className="list-group-item mb-3" key={comment._id}>
